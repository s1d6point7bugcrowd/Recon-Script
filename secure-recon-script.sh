...
# Define log file for forensic timestamped records

LOG_FILE="./scan_activity.log"

ENCRYPTED_LOG_FILE="./scan_activity.log.enc"



# Function for forensic logging
...
function announce_message() {
    ...
    log_activity "$message"  # Log the message
...
log_activity "Voice announcements enabled: $ENABLE_VOICE"
...
log_activity "Store data permanently: $STORE_PERMANENTLY"
...
if [ "$STORE_PERMANENTLY" == "n" ]; then
    ...
    log_activity "Data directory set to $DATA_DIR"
...
log_activity "Use proxychains: $USE_PROXYCHAINS"
...
log_activity "Use Nuclei cloud features: $USE_NUCLEI_CLOUD"
...
log_activity "Scan type selected: $SCAN_TYPE"
...
if [[ "$SCAN_TYPE" != "1" && "$SCAN_TYPE" != "2" ]]; then
    ...
    log_activity "Invalid scan type selected"
...
log_activity "Out-of-scope patterns: $OOS_INPUT"
...
CUSTOM_HEADER="x-bugcrowd-research: researcher@$PROGRAM_NAME"

log_activity "Bug bounty program name: $PROGRAM_NAME"
...
log_activity "Nuclei template paths: ${TEMPLATE_PATHS_ARRAY[*]}"
...
log_activity "Nuclei template tags: ${TEMPLATE_TAGS_ARRAY[*]}"
...
log_activity "Nuclei severity levels: $SEVERITY_LEVELS"
...
log_activity "Nuclei rate limit: $RATE_LIMIT"
...
log_activity "DNSX wordlist: $DNSX_WORDLIST"
...
log_activity "DNSX resolver list: $RESOLVER_LIST"
...
log_activity "HTTPX status codes: $STATUS_CODES"
...
function find_new_subdomains() {
    ...
    if [ -s "$output_file" ]; then
        ...
        log_activity "New subdomains discovered"

    else

        echo -e "${CYAN}No new subdomains discovered by dnsx.${NC}"

        log_activity "No new subdomains discovered by dnsx"
...
if [[ "$SCAN_TYPE" -eq 1 ]]; then
    ...
    log_activity "Target domain: $TARGET"
    ...
    log_activity "Invalid scan type selection"
...
log_activity "Nuclei scan completed."

echo -e "${GREEN}Nuclei scan completed.${NC}"



# Encryption part for forensic logs

announce_message "Encrypting scan logs..."

echo -e "${ORANGE}Enter a passphrase for encrypting the log file:${NC}"

read -s PASSPHRASE



openssl enc -aes-256-cbc -salt -in "$LOG_FILE" -out "$ENCRYPTED_LOG_FILE" -pass pass:"$PASSPHRASE"

log_activity "Encrypted log file generated: $ENCRYPTED_LOG_FILE"



# Clean up unencrypted log files

rm "$LOG_FILE"

log_activity "Unencrypted log file removed"

announce_message "Nuclei scan and encryption completed."

echo -e "${GREEN}Nuclei scan and encryption completed.${NC}"
