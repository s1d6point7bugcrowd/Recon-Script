...
if [ -z "$DNSX_WORDLIST" ]; then
    DNSX_WORDLIST="/root/SecLists/Discovery/DNS/subdomains-top1million-5000.txt"
...
if [ -z "$RESOLVER_LIST" ]; then
    RESOLVER_LIST="/root/resolvers/resolvers-trusted.txt"
...
function run_nuclei() {
    local target_file=$1
    local nuclei_cmd="nuclei -rl $RATE_LIMIT -H \"$CUSTOM_HEADER\" $SEVERITY_FLAG $CLOUD_UPLOAD_FLAG"
...
if [[ "$SCAN_TYPE" -eq 1 ]]; then
    ...
    $PROXYCHAINS_CMD subfinder -d $TARGET -silent -all > "${DATA_DIR}/${TARGET}-subs.txt"
    ...
    DNSX_OUTPUT="${DATA_DIR}/${TARGET}-dnsx-results.txt"

    $PROXYCHAINS_CMD dnsx -list "${DATA_DIR}/${TARGET}-filtered-subs.txt" -r "$RESOLVER_LIST" -rl 5 -resp -silent -o "$DNSX_OUTPUT"

    # Check if dnsx found any new subdomains
    if [ -s "$DNSX_OUTPUT" ]; then
        echo -e "${ORANGE}New subdomains discovered by dnsx. Combining with subfinder results...${NC}"
        cat "$DNSX_OUTPUT" >> "${DATA_DIR}/${TARGET}-filtered-subs.txt"
        sort -u "${DATA_DIR}/${TARGET}-filtered-subs.txt" > "${DATA_DIR}/${TARGET}-combined-subs.txt"
    else
        echo -e "${CYAN}No new subdomains discovered by dnsx. Using subfinder results only.${NC}"
        cp "${DATA_DIR}/${TARGET}-filtered-subs.txt" "${DATA_DIR}/${TARGET}-combined-subs.txt"
    fi
    ...
    httpx_output=$(echo $URL | $PROXYCHAINS_CMD httpx -silent -title -rl 5 -status-code -td -mc $STATUS_CODES -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
