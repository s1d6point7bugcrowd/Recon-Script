...
is_oos() {
    ...
    for pattern in "${OOS_PATTERNS[@]}"; do
        if [[ "$target" == *"$pattern"* ]] && [[ "$target" != "https://$pattern" ]] && [[ "$target" != "$pattern" ]]; then
            return 0
        fi
    done
    return 1
...
SUBZY_RESULTS_FILE=$(create_file "${TARGET}-subzy-results.json")
...
run_subfinder_dnsx_httpx() {
    ...
    echo -e "\033[33mRunning subzy for subdomain takeover detection...\033[0m"
    subzy run --targets $SUBS_FILE --output $SUBZY_RESULTS_FILE
    check_file_content "$SUBZY_RESULTS_FILE"
}
...
if [ "$MODE" == "domain" ]; then
    ...
    if is_oos "$TARGET"; then
        ...
        echo -e "\033[33mRunning httpx...\033[0m"
        cat $NUCLEI_READY_FILE | httpx -silent --rate-limit 5 -title -status-code -mc 200 | anew $WEB_ALIVE_FILE
        check_file_content "$WEB_ALIVE_FILE"

        # Remove duplicates
        sort -u $WEB_ALIVE_FILE -o $NUCLEI_READY_FILE
