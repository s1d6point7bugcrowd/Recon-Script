...
HTTPS_ONLY_FILE=$(create_file "$(sanitize_filename "https-only.txt")")
...
run_subfinder_dnsx_httpx() {
    ...
    dnsx -resp -a -silent < $FILTERED_SUBS_FILE > $ALIVE_SUBS_IP_FILE
    ...
    httpx -silent --rate-limit 5 -title -status-code -mc 200 < $ALIVE_SUBS_FILE | remove_brackets > $WEB_ALIVE_FILE
...
run_waybackurls() {
    local target=$1
    local wayback_output=$(create_file "$(sanitize_filename "wayback-$target.txt")")
    echo -e "\033[33mRunning waybackurls for: $target...\033[0m"
    echo "$target" | waybackurls > "$wayback_output"
    # Filter the waybackurls output for OOS URLs
    filter_urls "$wayback_output" $WAYBACK_URLS_FILE
}
...
if [ "$MODE" == "domain" ]; then
    ...
    if [ "$USE_WAYBACKURLS" == "yes" ]; then
        while read -r url; do
            run_waybackurls "$url"
        done < $NUCLEI_READY_FILE
    fi

    if [ "$USE_NAABU" == "yes" ]; then
        echo -e "\033[33mRunning naabu...\033[0m"
        sudo naabu -top-ports 1000 -silent < $ALIVE_SUBS_FILE > $OPEN_PORTS_FILE
        ...
        httpx -silent --rate-limit 5 -title -status-code -mc 200 < $FINAL_OPEN_PORTS_FILE | remove_brackets > $WEB_ALIVE_FILE
        check_file_content "$WEB_ALIVE_FILE"
    ...
    if is_oos "$TARGET"; then
        ...
        echo -e "\033[33mRunning dnsx...\033[0m"
        echo "$TARGET" | dnsx -silent > $NUCLEI_READY_FILE
        check_file_content "$NUCLEI_READY_FILE"

        echo -e "\033[33mRunning httpx...\033[0m"
        cat $NUCLEI_READY_FILE | httpx -silent --rate-limit 5 -title -status-code -mc 200 > $TEMP_FILE
        check_file_content "$TEMP_FILE"
