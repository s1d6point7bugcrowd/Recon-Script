...
BASE_URLS_FILE=$(create_file "${TARGET}-base-urls.txt")
...
# Function to ensure unique base URLs
unique_base_urls() {
    local input_file=$1
    local output_file=$2
    extract_base_url < $input_file | sort -u > $output_file
    check_file_content $output_file
}
...
run_subfinder_dnsx_httpx() {
    ...
    unique_base_urls $WEB_ALIVE_FILE $BASE_URLS_FILE

    # No additional filtering needed, as httpx already filtered status codes 200 and 302
    cp $BASE_URLS_FILE $NUCLEI_READY_FILE
}
...
if [ "$MODE" == "domain" ]; then
    ...
    if [ "$USE_WAYBACKURLS" == "yes" ]; then
        ...
        unique_base_urls $FILTERED_WAYBACK_URLS_FILE $BASE_URLS_FILE

        echo -e "\033[33mRunning httpx on waybackurls...\033[0m"
        cat $BASE_URLS_FILE | httpx -silent -rate-limit 5 -title -status-code -td -mc 200 | anew $TEMP_FILE
