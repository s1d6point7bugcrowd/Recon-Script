...
TEMP_FILE=$(create_file "temp-file.txt")
...
# Function to track processed URLs and avoid duplicates

add_to_nuclei_ready() {

    local url=$1

    if ! grep -Fxq "$url" $NUCLEI_READY_FILE; then

        echo "$url" | anew $NUCLEI_READY_FILE
...
run_subfinder_dnsx_httpx() {
    ...
    sort -u $ALIVE_SUBS_IP_FILE -o $ALIVE_SUBS_IP_FILE
    ...
    sort -u $ALIVE_SUBS_FILE -o $ALIVE_SUBS_FILE
    ...
    sort -u $WEB_ALIVE_FILE -o $WEB_ALIVE_FILE

    check_file_content "$WEB_ALIVE_FILE"



    # Add URLs directly to nuclei ready file

    filter_urls $WEB_ALIVE_FILE $NUCLEI_READY_FILE
...
if [ "$MODE" == "domain" ]; then
    ...
    if [ "$USE_NAABU" == "yes" ]; then
        ...
        sort -u $OPEN_PORTS_FILE -o $OPEN_PORTS_FILE
        ...
        sort -u $FINAL_OPEN_PORTS_FILE -o $FINAL_OPEN_PORTS_FILE
    ...
    if [ "$USE_WAYBACKURLS" == "yes" ]; then
        ...
        sort -u $WAYBACK_URLS_FILE -o $WAYBACK_URLS_FILE
        ...
        echo -e "\033[33mFiltering OOS waybackurls...\033[0m"

        filter_waybackurls $FILTERED_WAYBACK_URLS_FILE $FILTERED_WAYBACK_URLS_FILE



        echo -e "\033[33mRunning httpx on waybackurls...\033[0m"

        cat $FILTERED_WAYBACK_URLS_FILE | httpx -silent -rate-limit 5 -title -status-code -td -mc 200 | anew $TEMP_FILE

        sort -u $TEMP_FILE -o $TEMP_FILE



        # Find new URLs and use comm to remove duplicates

        comm -23 <(sort -u $TEMP_FILE) <(sort -u $NUCLEI_READY_FILE) | anew $NEW_URLS_FILE

        sort -u $NEW_URLS_FILE -o $NEW_URLS_FILE
        ...
        filter_urls $NEW_URLS_FILE $NUCLEI_READY_FILE
